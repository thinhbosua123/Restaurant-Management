...
public class Bill_Form extends javax.swing.JPanel {
    ...
    try {
        if (customer == null) return; // Tránh lỗi NullPointerException
        list = service.getListHD(customer.getID_KH());
        tableHD.removeAllRow(); // Đảm bảo không bị trùng dữ liệu
        for (ModelHoaDon data : list) {  
            tableHD.addRow(new Object[]{
                data.getIdHoaDon(), data.getNgayHD(), df.format(data.getTienMonAn()) + "đ",
    ...
    tableHD.removeAllRow(); // Xoá tất cả dữ liệu cũ trước khi tìm kiếm
    for (ModelHoaDon data : list) {
